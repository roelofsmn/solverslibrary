...
namespace Solvers.AssignmentProblem
{
    public static class AssignmentProblem
    {
        ...
        public static IEnumerable<Tuple<int, int>> HungarianAlgorithm(double[,] costMatrix)
        {
            ...
            var newCostMatrix = costMatrix.Copy();
        ...
        private class State
        {
            ...
            {
                var C = DenseMatrix.OfArray(CostMatrix);

                CostMatrix = SubtractFromRows(CostMatrix, RowMinima(C));
            }
            public void SubtractColumnMinima()
            {
                var C = DenseMatrix.OfArray(CostMatrix);

                CostMatrix = SubtractFromColumns(CostMatrix, ColumnMinima(C));
            }
        }
        ...
        {
            var S = new List<Tuple<int, int, Marks>>();
            ...
            foreach (var t in S)//.Where(s => s.Item3 == Marks.Star))
                state.Marked.RemoveAll(m => t.Equals(m));
        ...
        private static double[,] SubtractFromRows(double[,] matrix, double[] rowValues)
        {
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
                for (int j = 0; j < matrix.GetLength(1); j++)
                    result[i, j] -= rowValues[i];

            return result;
        }
        private static double[,] SubtractFromColumns(double[,] matrix, double[] columnValues)
        {
            var result = matrix.Copy();
        ...
        private static double[] RowMinima(DenseMatrix M)
        {
            var result = new double[M.RowCount];
            for (int i = 0; i < M.RowCount; i++)
            {
                result[i] = M.Row(i).Minimum();
            }
            return result;
        }
        private static double[] ColumnMinima(DenseMatrix M)
        {
            var result = new double[M.ColumnCount];
            for (int i = 0; i < M.ColumnCount; i++)
            {
                result[i] = M.Column(i).Minimum();
            }
            return result;
        }
